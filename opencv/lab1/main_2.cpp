...
int main() {
    ...
    Mat proj_oy = Mat::zeros(I.rows, 1, CV_32F);
    for (int i = 0; i < I.rows; i++) {
        float sum = 0;
        for (int j = 0; j < I.cols; j++) {
            sum += I.at<uchar>(i, j);
        }
        proj_oy.at<float>(i) = sum;
    }

    // Горизонтальная проекция (на Ox)
    Mat proj_ox = Mat::zeros(1, I.cols, CV_32F);
    for (int j = 0; j < I.cols; j++) {
        float sum = 0;
        for (int i = 0; i < I.rows; i++) {
            sum += I.at<uchar>(i, j);
        }
        proj_ox.at<float>(0, j) = sum;
    }

    // Нормализация проекций для отображения
    double proj_oy_max, proj_ox_max;
    minMaxLoc(proj_oy, nullptr, &proj_oy_max);
    minMaxLoc(proj_ox, nullptr, &proj_ox_max);

    // Создаем изображения для проекций
    Mat proj_oy_img = Mat::ones(256, I.cols, CV_8UC3) * 255;
    Mat proj_ox_img = Mat::ones(I.rows, 256, CV_8UC3) * 255;

    // Нормализуем проекции для графического отображения
    for (int i = 0; i < proj_oy.rows; i++) {
        int normalized_value = cvRound(proj_oy.at<float>(i) * proj_oy_img.cols / proj_oy_max);
        line(proj_oy_img, Point(0, i), Point(normalized_value, i), Scalar(0, 0, 0), 1);
    }

    for (int j = 0; j < proj_ox.cols; j++) {
        int normalized_value = cvRound(proj_ox.at<float>(0, j) * proj_ox_img.rows / proj_ox_max);
        line(proj_ox_img, Point(j, 0), Point(j, normalized_value), Scalar(0, 0, 0), 1);
    }
    ...
    imshow("Вертикальная проекция", proj_oy_img);
    imshow("Горизонтальная проекция", proj_ox_img);
